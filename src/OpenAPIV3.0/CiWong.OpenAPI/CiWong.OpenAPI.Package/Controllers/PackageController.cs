...
namespace CiWong.OpenAPI.ToolsAndPackage.Controllers
{
	public class PackageController : ApiController
	{
        ...
        public object book_resources(long packageId,string cId)
	    {
            ...
            var taskResultContent = packageCategoryContent.TaskModules.ToDictionary(c => c.ModuleId, c => packageCategoryContent.ResultContents.Where(x => x.ModuleId == c.ModuleId).ToList());
            ...
            var jsonData = taskResultContent.Select(t => new
            {
                moduleInfo = new 
                {
                    packageCatalogueId = t.Value.First().PackageCatalogueId,
                    moduleId = t.Value.First().ModuleId,
                    moduleName = t.Value.First().ModuleName
                },
